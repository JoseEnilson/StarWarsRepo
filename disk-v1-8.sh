...
# Melhores práticas: Google Gemini
# ------------------------------------------------------------------------ #
...
function success_message() {
    ...
    echo -e "${C_GREEN}\n***** CONFIGURAÇÃO EXECUTADA COM SUCESSO *****${C_RESET}\n" | tee -a "$DISK_LOG_FILE"
...
function add_disk_to_lvm() {
    ...
    echo -e "${C_GREEN}Disco configurado e montado com sucesso!${C_RESET}" | tee -a "$LVM_LOG_FILE" # Mensagem para tela e log
}
